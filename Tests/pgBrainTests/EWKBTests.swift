...
final class EWKBTests: XCTestCase {
    ...
    /// Concatenate byte chunks without a long `+` chain — a chain of `+` over
    /// `[UInt8]` makes the Swift type-checker explore operator overloads and
    /// time out on some toolchains ("unable to type-check in reasonable time").
    ...
    func testLineString() {
        let bytes = cat([[1], u32(2), u32(2), f64(1), f64(2), f64(3), f64(4)])
        XCTAssertEqual(EWKB.toEWKT(buf(bytes)), "LINESTRING(1 2, 3 4)")
    }

    func testPolygon() {
        // one ring, four points (closed)
        let ring = cat([u32(4), f64(0), f64(0), f64(1), f64(0), f64(1), f64(1), f64(0), f64(0)])
        let bytes = cat([[1], u32(3), u32(1), ring])
        XCTAssertEqual(EWKB.toEWKT(buf(bytes)), "POLYGON((0 0, 1 0, 1 1, 0 0))")
    }

    func testMultiPoint() {
        let bytes = cat([[1], u32(4), u32(2), point(1, 2), point(3, 4)])
        XCTAssertEqual(EWKB.toEWKT(buf(bytes)), "MULTIPOINT(1 2, 3 4)")
    }

    func testMultiLineString() {
        let line = cat([[1], u32(2), u32(2), f64(0), f64(0), f64(1), f64(1)])
        let bytes = cat([[1], u32(5), u32(1), line])
        XCTAssertEqual(EWKB.toEWKT(buf(bytes)), "MULTILINESTRING((0 0, 1 1))")
    }

    func testMultiPolygon() {
        let ring = cat([u32(4), f64(0), f64(0), f64(1), f64(0), f64(1), f64(1), f64(0), f64(0)])
        let poly = cat([[1], u32(3), u32(1), ring])
        let bytes = cat([[1], u32(6), u32(1), poly])
        XCTAssertEqual(EWKB.toEWKT(buf(bytes)), "MULTIPOLYGON(((0 0, 1 0, 1 1, 0 0)))")
    }

    func testGeometryCollection() {
        let bytes = cat([[1], u32(7), u32(2), point(1, 2), point(3, 4)])
    ...
    func testBigEndianPoint() {
        ...
        let bytes = cat([[0], u32be(1), f64be(1), f64be(2)])
    ...
    func testLeftoverBytesRejected() {
        XCTAssertNil(EWKB.toEWKT(buf(cat([point(1, 2), [0xFF]]))), "trailing bytes ⇒ not a clean geometry")
    }

    func testUnknownTypeRejected() {
        XCTAssertNil(EWKB.toEWKT(buf(cat([[1], u32(99), f64(1), f64(2)]))))
    }

    func testTruncatedRejected() {
        XCTAssertNil(EWKB.toEWKT(buf(cat([[1], u32(1), f64(1)]))), "missing the second coordinate")
